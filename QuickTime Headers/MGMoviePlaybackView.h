//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "MGVideoView.h"

@class MGMoviePlaybackController, QTMovieLayer;

@interface MGMoviePlaybackView : MGVideoView
{
    MGMoviePlaybackController *_playbackController;
    QTMovieLayer *_movieLayer;
}

- (void)observeValueForKeyPath:(id)arg1 ofObject:(id)arg2 change:(id)arg3 context:(void *)arg4;
- (void)playbackControllerMovieNaturalSizeDidChange:(id)arg1;
@property(retain, nonatomic) MGMoviePlaybackController *playbackController;
- (void)dealloc;
- (id)initWithFrame:(struct CGRect)arg1 videoLayer:(id)arg2;
- (id)initWithFrame:(struct CGRect)arg1;

@end

